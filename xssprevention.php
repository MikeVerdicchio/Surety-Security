	...
	<div class="container-fluid">
		...
		<div class="col-md-8 col-md-offset-2">
			<div class="row">
				...
				for the specified input. For example, a zip code only requires a user to
				input digits from 0-9. There is no reason for any special characters like
				a &lt; or a " to be allowed in the input. The text box itself can limit the 
				input or the code can just strip anything that isn't a digit from 0-9
				when the user submits.
				<h3>Blacklisting</h3>
				Blacklisting compares the user input with a list of untrusted inputs. For example,
				not allowing any text coming in containing the script tag. This shouldn't be relied upon as 
				the main form of validation becuase there can be always something not accounted for on the list
				that is malicious.
			</div>
			<div class="row">
				<h2>HTML Entity Encoding</h2>
				There are some characters that can be used to manipulate a html page, like &lt; &gt; or ' that
				need to be encoded so that they aren't interpreted as the browser as code. This is done by
				using HTML character entities. For example, &lt; should be encoded as &amp;lt; and &gt; should be encoded by
			...
			<div class="row">
				...
				in an unintended way. Having one place on a site that doesn't
				properly format untrusted data can lead to the website being compromised by an XSS attack.
			</div>
			<div class="row">
